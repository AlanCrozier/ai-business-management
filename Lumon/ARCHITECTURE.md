...
sequenceDiagram
    participant User
    participant Frontend as LUMON React App (Vite)
    ...
    Backend->>DB: Looks up user's encrypted Service Account
    DB-->>Backend: Returns Service Account JSON (decrypted in memory)
    
    Backend->>Sheets: Authenticates via Service Account
    ...
    Frontend->>User: Renders Dashboard, Graphs, and Enterprise KPI Cards
```
...
*   **Secure Database Communication:** It uses the user's decrypted `service_account.json` to securely talk to Google Sheets via the Google API. It fetches data, filters out everything that doesn't match the active user's UID, and formats it for the frontend.
...
*   **MySQL (Internal DB):** Powered by SQLAlchemy, this relational database stores internal application state, such as `UserAccount` mapping (linking a Firebase UID to an encrypted Google Service Account JSON) and auto-seeded local datasets.
*   **Google Sheets (External Data):** Google Sheets acts as the user's cloud database. It is segmented into dedicated tables (`Sales_Data`, `Customer_Data`, `Employee_Data`, `Predictions_Data`). Not only does it store historical data, but the backend automatically writes the newly generated AI predictions back into the sheet, ensuring a permanent historical record of system forecasts and anomalies.

---

*In short: React handles the visuals, Firebase handles the security, FastAPI routes the traffic, MySQL manages internal configuration, Google Sheets handles user data storage, and the ML models act as the oracle that predicts the future.*
