...
import { Activity, Users, Target, Zap, CloudOff } from 'lucide-react';
import { Line, Bar, Doughnut } from 'react-chartjs-2';
import { KPICard } from '../KPICard';

export default function OverviewTab({ apiData }: any) {
  const hasData = apiData && apiData.dashboardSummary;
  const summary = hasData ? apiData.dashboardSummary : {};
  ...
  let p = 0, g = 0, s = 0, st = 0, v = 0;
  custs.forEach((c: any) => {
      if (c.membership_level === 'Platinum') p++;
      else if (c.membership_level === 'Gold') g++;
      else if (c.membership_level === 'Silver') s++;
      else if (c.membership_level === 'VIP') v++;
      else st++; // Default to Standard for any other or 'Standard'
  });
  const doughnutData = {
    labels: ['Platinum', 'Gold', 'Silver', 'VIP', 'Standard'],
    datasets: [{
      data: [p, g, s, v, st],
      backgroundColor: ['#8B9D83', '#C4A484', '#2D3436', '#D3C4B3', '#A3B19B'],
      borderWidth: 0
    }]
  };
    ...
    >
      ...
      {!hasData && (
        <div className="flex items-center gap-3 p-4 bg-amber-50 rounded-xl border border-amber-200">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-800">Waiting for data sync</p>
            <p className="text-xs text-amber-600 mt-0.5">Add data via Sales HQ, Customers, or Employees tabs — it will appear here automatically.</p>
          </div>
        </div>
      )}
